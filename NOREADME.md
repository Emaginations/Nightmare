...
try17-------------------------------------------------try21
现在有新要求:1.在无差别催睡webui配置的白名单后面添加一个群聊白名单，并增加相关实现逻辑。2.请参考TaiYang的戳一戳插件，现在llmprovider已经能够正常生成催睡，只是没有通过maisaka循环发送啊，建议是：使用新的hook方法拦截本次原本应该发送的消息，不要交给maisaka处理，直接用hook进行消息发送。请参考：生成Try21并修改不合理的地方：
...
这个菲比秋比可能是对的，听她的修改22
-----------------------------------------------------try25
现在生成try25，要求：1.更改一下LLM传出的语句，催睡对象的名字出现的概率为：私聊是0.8，无差别开启时是0.3，并且两次催睡间隔越短名字出现的频率越小，最小为0.01。前置名字和后置名字的概率分别为一半。2.在催睡设置里增加沉默选项UI以及逻辑，默认关闭，逻辑是开启时hook截断消息并不发送任何消息。添加注释为：沉默...是最好的陪伴，并发送同样的maibot日志。3.更改LLM隐藏附带的时间为应该催睡的时间加现在的时间。