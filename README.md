...
### ✅写完了
### ✅测完了
### 🛠️维护中。如有疑问请联系作者[1m](https://qm.qq.com/q/331701160)或[插件开发群：](https://qm.qq.com/q/1036092828)
...
- **用户白名单**：在无差别模式下免受催睡的用户ID列表。
- **群聊白名单**：仅在列表中的群聊才会触发催睡，留空表示所有群聊均可触发。
...
3. **目标用户判定**：
   - 若开启**无差别催睡**，则用户白名单外的所有用户都会被催睡。
   - 若未开启，仅对配置的`target_user`进行催睡。
4. **群聊白名单检查**：若设置了群聊白名单，则仅在列表内的群聊中触发催睡；未设置则所有群聊均可触发。
...
9. **发送并拦截**：通过`self.ctx.send.text`直接发送催睡消息，然后返回`{"action": "abort"}`阻止原消息继续被 Maisaka 处理，避免重复响应。
10. **状态持久化**：用户的互动时间和催睡时间保存在`nightmare_state.json`中，重启不丢失。
